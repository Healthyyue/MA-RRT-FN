classdef MARRTsFN
    ...
    properties
        ...
        edges               % 树中各节点的边长
        nodes               % 树中的节点
        parent              % 节点的父节点,记录节点的父节点在树中的位置
        child               % 节点的孩子节点，记录节点的孩子节点的个数
...
%         numnodes            % 树中节点数
    ...
    methods
        function rrt = MARRTsFN(map,position,goal,Agent_count)                           % 初始化RRT算法
            %UNTITLED2 构造此类的实例
            %   此处显示详细说明
            ...
            rrt.child = 0;                                                          % 孩子节点数组初始化
            rrt.max_runtime = 5.000;                                                % RRT最长运行时间为5秒
            rrt.possibility = 0.1;                                                  % 采样点为目标点的概率为0.1
            rrt.max_nodes = 1000;                                                    % 树中节点数最大限制
            rrt.maxiterations = 5000;
        end
        
        function rrt = runRRTfn(rrt,single_solution)                                % 运行RRT算法
            ...
            while true
                goal_index = 0;                                                     % 目标节点在树中的下标初始化为0
                if rrt.max_nodes < (size(rrt.nodes,1)+1)                            % 如果节点数再增加一个就大于最大节点数
                    old_tree_nodes = rrt.nodes;                                     % 则先备份原来的树
                    old_tree_edges = rrt.edges;
                    old_tree_parent = rrt.parent;
                    old_tree_cost = rrt.cost;
                    old_tree_child = rrt.child;
                end
                if rand() < rrt.possibility                                         % 如果产生的随机数小于设定概率值且循环次数大于500
                ...
                rrt.child = extend.child;                                           % 更新扩展得到的新树各节点的孩子节点数组
                if rrt.max_nodes < size(rrt.nodes,1)                                % 如果树中的节点数仍然大于最大节点数（说明rewire过程没有删掉节点）
                    for i = 1:(size(rrt.nodes,1)-1)                                 % 对树中每一个节点，除新节点和目标点外
%                          if DIST(rrt,rrt.nodes(i,:),rrt.goal) < rrt.c_max          % 不删除目标节点周围的节点
                        if isequal(rrt.nodes(i,:),rrt.goal)                         % 不删除目标节点
                            continue;
                        end
                        if rrt.child(i,:) == 0
                            rrt.nodes(i,:) = [];
                            rrt.edges(i,:) = [];
                            rrt.cost(i,:) = [];               
                            rrt.child(rrt.parent(i),:) = rrt.child(rrt.parent(i),:) - 1;   % 更新该节点父节点的孩子节点个数
                            rrt.child(i,:) = [];                                    % 删除该节点的孩子节点参数
                            value_change_position = (rrt.parent > i);                   % 找到所有大于该节点在树中的索引值的Parent数值
                            rrt.parent(value_change_position) = rrt.parent(value_change_position)-1;    % 更新这些Parent数值，全部减1
                            rrt.parent(i,:) = [];                                   % 删除该节点指向其父节点的Parent值
                            break;
                        end
                    end  
                end
                if rrt.max_nodes < size(rrt.nodes,1)                                % 如果没有删除节点的程序执行
                    rrt.nodes = old_tree_nodes;                                     % 则返回原来的树
                    rrt.edges = old_tree_edges;
                    rrt.parent = old_tree_parent;
                    rrt.cost = old_tree_cost;
                    rrt.child = old_tree_child;
                end
...
%                 end
        ...
        function rand_node = SAMPLE(rrt,single_solution)                          % 在联合状态空间随机采样一个节点                                                       
            ...
            if nargin == 2                                                                      % 如果输入参数数目等于2
                ...
                rand_cost = ceil(rand()*max_cost+0.5);                                          % 在最大代价范围内随机选择一个代价值
        ...
        function extend = EXTEND(rrt,nodes,edges,x)                                 % 向随机采样点x扩展
            ...
            if ~isempty(p_new)                                                      % 如果到新节点有边
                ...
                if alreadyintree == 1                                               % 如果新节点已经在树中
                    ...
                    extend.child = Child;                                               % 返回新树的child数组
                    extend.cost = Cost;                                                 % 返回新树的Cost数组
                    return;                                                         % 则不必进行更新
                end
                % 画图
%                 if size(tree_nodes,1) >= 200
%                     if rrt.agent_count == 1
%                         figure(2);
%                         clf(figure(2),'reset');
%                         ylim manual
...
%                     plot(x_new(1,1),x_new(1,2),'bo','MarkerSize',2,'MarkerFaceColor','r');
                ...
                Child(Parent(end,:),:) = Child(Parent(end,:),:) + 1;                % 找到x_new的父节点
...
%                 line(x,y,'Color','cyan','LineWidth',3);
                % 画图
                if ~isempty(x_near.nodes)                                           % 如果邻近节点集非空
                    jump_index = flag;                                              % 跳过的序号初始化
                    for i = 1:size(x_near.nodes,1)                                  % 对以x_new为中心的邻域内的所有节点
                        if ~all(i ~= jump_index)                                    % 如果jump_index中有值等于i
                            continue;                                               % 则跳过当前循环
                        end
                        ...
                        if isequal(new_near.x_new,x_near.nodes(i,:)) && COST(rrt,Parent,Cost,x_near.index(i)) > c_new + new_near.cost  % 如果扩展得到的新点是x_near节点并且以x_new作为x_near的父节点得到的代价比x_near在树中的原代价小
                            if Child(Parent(x_near.index(i)),:) == 1 && rrt.max_nodes < size(tree_nodes,1) && ... % 当邻近节点的父节点只有一个孩子节点且达到最大节点限制
                                DIST(rrt,tree_nodes(Parent(x_near.index(i)),:),rrt.goal) > rrt.c_max % 且邻近节点的父节点不在目标节点的周围，则
                                index = x_near.index(i); 
                                tree_nodes(Parent(index),:) = [];                   % 删除该x_near的父节点
                                tree_edges(Parent(index),:) = [];                   % 删除该父节点指向其父节点的边
                                Cost(Parent(index),:) = [];                         % 删除该父节点指向其父节点的边代价
                                Child(Parent(Parent(index)),:) = Child(Parent(Parent(index)),:) - 1; % 找到删除节点的父节点并令该父节点的孩子节点个数减1
                                Child(Parent(index),:) = [];                        % 删除该父节点的孩子节点参数
                                need_jump = find(x_near.index == Parent(index),1);  % 在邻域里找已经删除的点,返回该点在邻域节点集中的下标
                                if need_jump                                        % 如果邻域里有已经删除的点
                                    jump_index = [jump_index;need_jump];            % 记录该删除节点在邻域节点集中的下标
                                end
                                x_near.index(x_near.index > Parent(index)) = x_near.index(x_near.index > Parent(index)) - 1; % 邻近节点在树中的下标更新
                                value_change_position = (Parent > Parent(index));   % 找到所有大于该父节点在树中的索引值的Parent数值
                                Parent(value_change_position) = Parent(value_change_position)-1; % 更新这些Parent数值，全部减1
                                Parent(Parent(index),:) = [];                       % 删除该父节点指向其父节点的Parent值
                            else
                                Child(Parent(x_near.index(i))) = Child(Parent(x_near.index(i))) - 1;
                            end    
                            Parent(x_near.index(i)) = size(Parent,1);               % x_near的父节点设置为最后一个节点x_new的索引
                            Child(size(Parent,1),:) = Child(size(Parent,1),:)+1;    % x_new的孩子节点加1
