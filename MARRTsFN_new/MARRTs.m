...
%             rrt.optisol = sum(abs(position - goal));                                % 最好的解为欧式距离
        ...
        function rrt = runMARRTs(rrt,single_solution)                                % 运行RRT算法
            ...
            d_map = rrt.map;                                                        %画图数据初始化
            d_map(end+1,end+1) = 0;
            colormap([1 1 1;0 0 0;1 0 0;0.3 0.4 0.4;0 0 1]);  % 创建颜色
            ...
            while true                                                                % 如果产生的随机数大于等于设定概率值
                ...
                for j = 1 : 2 : size(rrt.nodes,2)
                    x_1 = size(rrt.nodes,1);
                    d_map(rrt.nodes(x_1,j),rrt.nodes(x_1,j+1)) = 3;
                end
                for i = 1 : 2 : size(rrt.nodes,2)
                    d_map(rrt.init_position(1,i),rrt.init_position(1,i+1)) = 4;
                    d_map(rrt.goal(1,i),rrt.goal(1,i+1)) = 2;
                end
                pcolor(0.5:size(rrt.map,2)+0.5,0.5:size(rrt.map,1)+0.5,d_map);                      % 赋予栅格颜色
                set(gca,'XTick',1:size(rrt.map,1),'YTick',1:size(rrt.map,2));           % 设置坐标
