...
classdef RRTs
    ...
    methods
        function rrt = RRTs(map,position,goal,Agent_count)                          % 初始化RRT算法
            %UNTITLED2 构造此类的实例
            %   此处显示详细说明
...
%            for i = 1:(ceil(rrt.n*(9/10)))
            while true                                                                % 如果产生的随机数大于等于设定概率值
                ...
                time_consume = toc;                                                 % 运行时间计算
                if (time_consume >= runtimelimit)                                   % 如果运行时间大于最大运行时间
                    flag = 0;
...
%                     end
% 	                rrt.path = Path;                                                % 最终得到的路径存储在rrt的path中
%                     for k = 1 : size(Path,1)                                        % 对除根节点的其余节点
%                         for m = 1 : size(Path{k,:},1)
%                             for l = 1:2:size(Path{k,:},2)
%                                 path_node = Path{k,:}(m,l:(l+1));
%                                 rrt.map(path_node(1),path_node(2)) = -(l+1)/2;
%                             end
%                         end
%                     end
%                     rrt.pathcost = path_cost;
%                     rrt.numiterations = num_iterations;
%                  	return;
%                 end
%                 fprintf('number of iterations = %d\n',iterations);
%                 if iterations >= 10
% %                     if isequal(goal_index,0)                                        % 找到初始的路径，以后每次迭代不重新计算路径
%                         flag = 0;
%                         for i = 1:size(rrt.nodes,1)                                             % 对树中的每一个节点
%                             if isequal(rrt.nodes(i,:),rrt.goal)                                 % 如果该节点和目标节点相等
%                                 goal_index = i;                                                 % 目标节点在树中的下标
...
%                         end
% %                     end
% %                     if isequal(goal_index,0)                                                    % 如果目标节点在树中下标为0，则进行以下查找
%                     if flag == 0
%                         for i = 2:size(rrt.edges,1)                                             % 对树中的每一条边
%                             for j = 1:size(rrt.edges{i})                                        % 对这条边上的每一个路径点
%                                 if isequal(rrt.edges{i}(j),rrt.goal)                            % 如果该路径点等于目标节点
...
%                         end
%                     end
%                     if isequal(goal_index,0)
%                         fprintf('number of iterations = %d\n',iterations);                      % 如果目标点没并入树中，输出迭代次数
%                     end
%                     if ~isequal(goal_index,0)
%                         path_cost = [path_cost,COST(rrt,rrt.parent,rrt.cost,goal_index)];                           % 存储每次迭代后的目标到根节点的代价
%                         num_iterations = [num_iterations,iterations];                           % 存储当前的迭代次数
%                         fprintf('number of iterations = %d, cost of path = %d\n',iterations,COST(rrt,rrt.parent,rrt.cost,goal_index));
%                         %drawnow;
%                     end
% %                     if ~isequal(goal_index,0)                                           % 当目标点已经并入树中时，每次迭代重新计算路径
% %                         index = goal_index;                                             % 存储目标节点在树中的下标
% %                         path_index = [];                                                % 下标序列作为路径初始化
% %                         while index ~= 0                                                % 直到并入根节点，循环结束
% %                             path_index = [path_index;index];                            % 将目标节点在树中的下标并入路径数组
                ...
                for i = 1 : rrt.agent_count                                                     % 对每一个智能体找到其随机路径点
                    ...
                    if ~isempty(single_solution(i).path)                                        % 如果解路径不为空
                        for j = 1 : size(single_solution(i).path,1)                             % 对解的每一条边
                            ...
                            if cumul_cost > rand_cost && j == 1                                 % 如果第一条边以内发现累加代价大于随机代价
                                informed_rand((2*i-1):2*i) = single_solution(i).path{j}(rand_cost,:); % 则随机节点等于以rand_cost值为索引的路径点
                            end
                            if cumul_cost > rand_cost && j > 1 && j < size(single_solution(i).path,1)% 如果第一条边以外发现累加代价大于随机代价
                            ...
                            informed_rand(2*i-1) = informed_rand(2*i-1) + normrnd(0,1) * 0.5;   % 随机点行位置在原位置偏移一个正态分布随机数乘0.5的距离
                            informed_rand(2*i) = informed_rand(2*i) + normrnd(0,1) * 0.5;       % 随机点列位置在原位置偏移一个正态分布随机数乘0.5的距离
